...
struct seagull_virtual_keyboard {
	struct wl_list link;
	struct tinywl_server *server;
...
struct seagull_touch {
	struct wl_list link;
	struct tunywl_server *server;
	struct wlr_input_device *device;
};
...
void destroy_pointer(struct wl_listener *listener, void*data){
	struct seagull_pointer *pointer = 
		wl_container_of(listener,pointer,destroy);

	wlr_cursor_detach_input_device(pointer->server->cursor,pointer->device);
	wl_list_remove(pointer->destroy.link);
	wl_list_remove(pointer->device->link);
	wlr_input_device_destroy(pointer->device);
	free(pointer);
}
...
static void server_new_pointer(struct tinywl_server *server,
	...
	struct seagull_pointer *p = malloc(sizeof(struct seagull_pointer));
	p->device = device;
	p->server = server;
	p->destroy.notify = destroy_pointer;
	wl_signal_add(&p->device->events.destroy,&p->destroy);
}
...
static void server_new_tablet_tool(struct tinywl_server *server,
		 struct wlr_input_device *device){
...
void destroy_virtual_keyboard(struct wl_listener *listener, void*data){
	struct seagull_virtual_keyboard *keyboard =
		wl_container_of(listener,keyboard,destroy);
	struct  tinywl_server *server = keyboard->server;

	wl_list_remove(&keyboard->link);
	wl_list_remove(&keyboard->wlr_virtual_keyboard->link);
	wl_list_remove(&keyboard->wlr_virtual_keyboard->input_device.link);
	wlr_input_device_destroy(&keyboard->wlr_virtual_keyboard->input_device);
	free(keyboard->wlr_virtual_keyboard);
	free(keyboard);
}
...
void server_new_virtual_keyboard(struct wl_listener *listener, void*data){
	...
	struct seagull_virtual_keyboard *keyboard = malloc(sizeof(struct seagull_virtual_keyboard));
	keyboard->wlr_virtual_keyboard = wlr_keyboard;
	keyboard->server = server;
	keyboard->destroy.notify = destroy_virtual_keyboard;
	wl_signal_add(&wlr_keyboard->events.destroy,&keyboard->destroy);

	wlr_seat_set_keyboard(server->seat,&wlr_keyboard->input_device);
...
static void server_new_input(struct wl_listener *listener, void *data) {
	...
	case WLR_INPUT_DEVICE_TABLET_TOOL:
		server_new_tablet_tool(server,device);
		break;
...
static void process_cursor_motion(struct tinywl_server *server, uint32_t time) {
	...
	if (!view) {
		 ...
		 * default. 
		 * this is unecessary since cursor is always on top of home view
		 */
...
static void server_new_output(struct wl_listener *listener, void *data) {
	...
	char str[80];
	sprintf(str,"echo %d %d %d %d >> log",wlr_output->width,wlr_output->height,wlr_output->phys_width,wlr_output->phys_height);
	system(str);
...
static void server_new_xdg_surface(struct wl_listener *listener, void *data) {
	...
	wl_list_insert(&server->views, &view->link);
	wlr_xdg_toplevel_set_size(xdg_surface,1920,1080);
}
